    ...
    >
      ...
      <div
        className={`absolute top-0.5 w-5 h-5 sm:w-7 sm:h-7 bg-white dark:bg-gray-800 rounded-full shadow-md flex items-center justify-center transition-all duration-300 transform ${
          theme === "light" ? "left-0.5 bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-gray-800 dark:to-gray-700" : "left-8 bg-gradient-to-br from-emerald-50 to-green-50 dark:from-gray-700 dark:to-gray-800"
        }`}
      >
        <span className={`text-sm transition-all duration-300 ${theme === "light" ? "text-yellow-600 dark:text-yellow-400" : "text-emerald-600 dark:text-emerald-400"}`}>{theme === "light" ? "☀️" : "🌙"}</span>
      </div>
