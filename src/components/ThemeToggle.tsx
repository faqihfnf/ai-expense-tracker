...
export default function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      {/* Toggle Button */}
      <div
        className={`absolute top-[1px] w-5 h-5 sm:w-7 sm:h-7 bg-white dark:bg-gray-800 rounded-full shadow-md flex items-center justify-center transition-all duration-300 transform ${
          theme === "light" ? "left-0.5 bg-gradient-to-br from-yellow-50 to-orange-50 dark:from-gray-800 dark:to-gray-700" : "left-8 bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-gray-700 dark:to-gray-800"
        }`}
      >
        <span className={`text-sm transition-all duration-300 ${theme === "light" ? "text-yellow-600 dark:text-yellow-400" : "text-indigo-600 dark:text-indigo-400"}`}>{theme === "light" ? "☀️" : "🌙"}</span>
      </div>

      {/* Background Icons */}
      <div className="absolute inset-0 flex items-center justify-between px-1.5 pointer-events-none">
        <span className={`text-xs transition-opacity duration-300 ${theme === "light" ? "opacity-0" : "opacity-40"}`}>☀️</span>
        <span className={`text-xs transition-opacity duration-300 ${theme === "light" ? "opacity-40" : "opacity-0"}`}>🌙</span>
      </div>
    </button>
  );
}
